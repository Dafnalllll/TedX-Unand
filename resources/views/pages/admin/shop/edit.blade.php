...
<div class="flex min-h-screen bg-gradient-to-br from-gray-900 via-black to-red-900">
    ...
    <main class="flex-1 p-8 relative overflow-y-auto h-screen overflow-x-hidden">
        ...
        <div class="hidden md:block text-white max-w-none w-full px-8 lg:px-8">
            <h1 class="text-3xl font-bold mb-6 mt-12 md:mt-0 text-center md:text-left">Edit Merch</h1>
            @if ($errors->any())
            ...
            @endif
            <form id="shopFormDesktop" action="{{ route('dashboard.shop.shop.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-none w-full" novalidate>
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <label for="photo" class="flex items-center gap-3 cursor-pointer w-full">
                        ...
                        <input type="file" name="photo" id="photo" accept="image/*"
                            class="hidden"
                            onchange="document.getElementById('file-name-desktop').textContent = this.files[0]?.name || '{{ $item->photo ? basename($item->photo) : 'No file chosen' }}';" />
                    </label>
                    @if($item->photo)
                ...
                <div class="pt-4">
                    <div class="flex justify-between items-center w-full">
                        ...
                        <button type="submit"
                            ...
                            <span class="drop-shadow">Update Shop</span>
                        </button>
                    </div>
                </div>
            </form>
        ...
        <div class="block md:hidden w-full max-w-md mx-auto">
            <div class="bg-white/90 rounded-3xl shadow-2xl border-2 border-pink-400 p-4 mt-12 mb-8 relative overflow-hidden">
                ...
                <h2 class="text-2xl font-extrabold text-pink-700 mb-4 text-center drop-shadow">Edit Merch</h2>
                @if ($errors->any())
                ...
                @endif
                <form id="shopFormMobile" action="{{ route('dashboard.shop.shop.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5" novalidate>
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <label for="photo-mobile" class="flex items-center gap-3 cursor-pointer w-full">
                            ...
                            <input type="file" name="photo" id="photo-mobile" accept="image/*"
                                class="hidden"
                                onchange="this.parentNode.querySelector('span.text-gray-400').textContent = this.files[0]?.name || '{{ $item->photo ? basename($item->photo) : 'No file chosen' }}';" />
                        </label>
                        @if($item->photo)
                            <img src="{{ asset('storage/'.$item->photo) }}" alt="Merch Photo" class="w-24 mt-2 rounded" />
                        @endif
                    ...
                    <button type="submit"
                        class="w-full bg-gradient-to-r from-pink-500 via-yellow-400 to-pink-500 hover:from-yellow-400 hover:to-pink-700 text-black px-4 py-3 rounded-xl font-bold shadow-lg hover:scale-105 transition-all duration-200 text-base">
                        Update Shop Item
                    </button>
                    <a href="{{ route('dashboard.shop.shop') }}"
