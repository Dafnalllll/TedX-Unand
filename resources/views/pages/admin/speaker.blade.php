...
@section('title', 'Speaker | TEDxUniversitas Andalas')
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 hidden">
    <div class="bg-gradient-to-br from-gray-900 via-black to-yellow-300 rounded-2xl shadow-2xl border-2 border-yellow-300 max-w-md w-full p-8 relative">
        <button type="button" onclick="closeDeleteModal()"
            class="absolute top-4 right-4 flex items-center justify-center w-10 h-10 rounded-full border-2 border-yellow-300 bg-white text-yellow-400 hover:bg-yellow-300 hover:text-white text-2xl font-bold focus:outline-none transition-all duration-200">
            &times;
        </button>
        <div class="flex flex-col items-center">
            <div class="p-4 mb-4 flex items-center justify-center">
                <div class="bg-white rounded-full border-2 border-yellow-300 p-4 mb-4 shadow-lg flex items-center justify-center">
                    <img src="{{ asset('img/admin/delete.webp') }}" alt="Delete" class="w-12 h-12 object-contain" />
                </div>
            </div>
            <h2 class="text-2xl font-bold text-yellow-400 mb-2">Delete Speaker</h2>
            <p class="mb-6 text-gray-200 text-center">Are you sure you want to delete?<br>
                <span id="deleteSpeakerName" class="inline-block bg-white text-yellow-600 border border-yellow-300 rounded px-3 py-1 font-semibold mt-2"></span>
            </p>
            <form id="deleteForm" action="" method="POST" class="w-full flex flex-col gap-3">
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button type="submit" class="px-6 py-2 rounded bg-yellow-300 text-black hover:bg-yellow-400 font-bold shadow transition-all hover:scale-105 ">Delete</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function openDeleteModal(action, speakerName) {
        ...
        document.getElementById('deleteSpeakerName').textContent = speakerName;
    }
