...
<div class="flex min-h-screen bg-gradient-to-br from-gray-900 via-black to-red-900">
    ...
    <main class="flex-1 p-8 relative ml-64">
        ...
        <div class="text-white">
            ...
            <form id="speakerForm" action="{{ route('dashboard.speaker.speaker.update', $speaker->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-gray-800 p-6 rounded-lg shadow-lg" novalidate>
                @csrf
                @method('PUT')
                <div>
                    <!-- Speaker Name -->
                ...
                <div>
                    ...
                    <label for="photo" class="flex items-center gap-3 cursor-pointer w-full">
                        ...
                        <input type="file" name="photo" id="photo" accept="image/*"
                            class="hidden"
                            onchange="document.getElementById('file-name').textContent = this.files[0]?.name || '{{ $speaker->photo ? basename($speaker->photo) : 'No file chosen' }}';" />
                    </label>
                    @if($speaker->photo)
                ...
                <div class="flex justify-end">
                    <button type="submit" class="bg-yellow-300 hover:bg-yellow-500 text-black px-6 py-3 rounded-lg font-semibold shadow hover:scale-105 transition-all">Update Speaker</button>
                </div>
            </form>
        </div>
    </main>
...
document.addEventListener('DOMContentLoaded', function() {
    const speakerForm = document.querySelector('form[action="{{ route('dashboard.speaker.speaker.update', $speaker->id) }}"]');
    if (speakerForm) {
        speakerForm.addEventListener('submit', function(e) {
            ...
            if (!name || !description) {
                ...
                Swal.fire({
                    ...
                    text: 'Please fill in all fields before updating the speaker!',
