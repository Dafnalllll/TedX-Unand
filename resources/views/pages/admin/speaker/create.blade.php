...
<div class="flex min-h-screen bg-gradient-to-br from-gray-900 via-black to-red-900">
    ...
    <main class="flex-1 p-8 relative overflow-y-auto h-screen overflow-x-hidden">
        ...
        <div class="absolute right-0 top-0 w-72 h-72 bg-gradient-to-br from-yellow-400 via-red-500 to-transparent opacity-20 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute left-1/2 bottom-0 w-64 h-64 bg-gradient-to-tr from-red-500 via-yellow-400 to-transparent opacity-10 rounded-full blur-2xl pointer-events-none"></div>

        <!-- DESKTOP FORM -->
        <div class="hidden md:block text-white max-w-none w-full px-8 lg:px-8">
            <h1 class="text-3xl font-bold mb-6 mt-12 md:mt-0 text-center md:text-left">Create New Speaker</h1>
            @if ($errors->any())
            ...
            @endif
            <form id="speakerFormDesktop" action="{{ route('dashboard.speaker.speaker.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-none w-full" novalidate>
                @csrf
                <div class="mb-4">
                ...
                <div class="pt-4">
                    <div class="flex justify-between items-center w-full">
                        ...
                        <button type="submit"
                            ...
                            <span class="drop-shadow">Create Speaker</span>
                        </button>
                    </div>
                </div>
            </form>
        ...
        <div class="block md:hidden w-full max-w-md mx-auto">
            <div class="bg-white/90 rounded-3xl shadow-2xl border-2 border-yellow-400 p-4 mt-12 mb-8 relative overflow-hidden">
                ...
                <h2 class="text-2xl font-extrabold text-yellow-700 mb-4 text-center drop-shadow">Create Speaker</h2>
                @if ($errors->any())
                ...
                @endif
                <form id="speakerFormMobile" action="{{ route('dashboard.speaker.speaker.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5" novalidate>
                    @csrf
                    <div>
                    ...
                    <div>
                        <label for="photo-mobile" class="block text-sm font-bold mb-1 text-yellow-600">Speaker Photo</label>
                        <label for="photo-mobile" class="flex items-center gap-3 cursor-pointer w-full">
                            ...
                            <input type="file" name="photo" id="photo-mobile" accept="image/*"
                                class="hidden"
                                onchange="document.getElementById('file-name-mobile').textContent = this.files[0]?.name || 'No file chosen';" />
                        </label>
                    </div>
                    <button type="submit"
...
document.addEventListener('DOMContentLoaded', function() {
    // Desktop validation
    const speakerFormDesktop = document.getElementById('speakerFormDesktop');
    if (speakerFormDesktop) {
        speakerFormDesktop.addEventListener('submit', function(e) {
    ...
    // Mobile validation
    const speakerFormMobile = document.getElementById('speakerFormMobile');
    if (speakerFormMobile) {
        speakerFormMobile.addEventListener('submit', function(e) {
            const name = speakerFormMobile.querySelector('[name="name"]')?.value.trim();
            const description = speakerFormMobile.querySelector('[name="description"]')?.value.trim();
            const photo = speakerFormMobile.querySelector('[name="photo"]')?.files.length;
